...
public interface NotificationService {
    ...
    void setPinned(UUID userId, UUID notificationId, boolean pinned);

    /** Xoá hẳn các thông báo đã đọc (trừ tin đã ghim). */
    int deleteAllRead(UUID userId);

    /** Xoá theo id đã chọn của chính user (trừ tin đã ghim). */
    int deleteByIds(UUID userId, Collection<UUID> ids);
}
